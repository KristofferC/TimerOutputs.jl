...
function Base.show(io::IO, to::TimerOutput; allocations::Bool = true, sortby::Symbol = :time, linechars::Symbol = :unicode)
    ...
    print_header(io, Δt, Δb, ∑t, ∑b, name_length, true, allocations, linechars)
    for timer in sort!(collect(values(to.inner_timers)); rev = true, by = x -> sortf(x, sortby))
        _print_timer(io, timer, ∑t, ∑b, 0, name_length, allocations, sortby)
    end
    print_header(io, Δt, Δb, ∑t, ∑b, name_length, false, allocations, linechars)
end
...
function print_header(io, Δt, Δb, ∑t, ∑b, name_length, header, allocations, linechars)
    ...
    alloc_headers = allocations ? "  alloc   %tot  %alloc " : ""
    ...
    if header
        ...
        str_time =  center(string(prettytime(∑t)    , " / ", prettypercent(∑t, Δt)), strwidth(time_header))
        str_alloc = center(string(prettymemory(∑b)  , " / ", prettypercent(∑b, Δb)), strwidth(allocation_header))
...
function _print_timer(io::IO, to::TimerOutput, ∑t::Integer, ∑b::Integer, indent::Integer, name_length, allocations, sortby)
    ...
    for timer in sort!(collect(values(to.inner_timers)), rev = true, by = x -> sortf(x, sortby))
        _print_timer(io, timer, ∑t, ∑b, indent + 2, name_length, allocations, sortby)
    end
end
